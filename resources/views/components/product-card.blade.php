    ...
    <a href="{{ route('productos.mostrar', $producto->id) }}" style="text-decoration: none; color: inherit;">
        <div class="card-img-wrap">
            ...
            <img
                ...
                width="400"
                height="500"
                onerror="this.src='https://placehold.co/400x500/111/333?text=SIN+IMG'"
            >
        </div>
        <div class="card-body">
            <span class="card-cat">{{ $producto->category->name }} — {{ ucfirst($producto->category->gender) }}</span>
            {{-- El nombre del producto se accede mediante la propiedad 'name' del objeto $producto --}}
            <h3 class="card-name">{{ $producto->name }}</h3>
            {{-- Usamos un método del modelo (o un accessor) para mostrar el precio ya formateado --}}

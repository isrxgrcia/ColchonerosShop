    ...
    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 4rem; align-items: start;">
        ...
        <div class="info-container" style="position: sticky; top: 7rem;">
            ...
            <form action="{{ route('cliente.carrito.agregar') }}" method="POST" id="form-{{ $producto->id }}">
                @csrf
                {{-- Este input oculto guarda el ID del producto para enviarlo al carrito --}}
                <input type="hidden" name="producto_id" value="{{ $producto->id }}">
                <div style="margin-bottom: 2.5rem;">
                    <label class="title-section" style="display: block; margin-bottom: 1rem;">SELECCIONA TALLA</label>
                    <div style="display: flex; flex-wrap: wrap; gap: 0.75rem;">
                        {{-- Hacemos un bucle para mostrar las tallas disponibles desde el inventario --}}
                ...
                {{-- El botón llama a una función JS que gestiona la petición AJAX para añadir al carrito --}}
